...
var pdfjsLib = void 0;
if (typeof window !== 'undefined' && window['pdfjs-dist/allpdfjs/pdf']) {
  pdfjsLib = window['pdfjs-dist/allpdfjs/pdf'];
} else {
  pdfjsLib = require('../allpdfjs/pdf.js');
}
...
var PDFFindBar = function () {
  function PDFFindBar(options) {
    var _this = this;
    console.log(options.findField);

    var l10n = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : _ui_utils.NullL10n;

    _classCallCheck(this, PDFFindBar);
    var search = window.location.search.substring(1);
    var urlTextToSearch =JSON.parse('{"' + decodeURI(search).replace(/"/g, '\\"').replace(/&/g, '","').replace(/=/g,'":"') + '"}')
    var unProcessedString=urlTextToSearch.searchtext;
    var finalString=urlTextToSearch.searchtext.substr(1, unProcessedString.length-2);
    console.log(finalString);
    //console.log(urlTextToSearch.searchtext);
    //console.log(window.location.search);
    var newValues=document.getElementById('findInput');
    newValues.setAttribute("data-status", finalString.toString());
    newValues.setAttribute("value", finalString.toString());
    window.PDFViewerApplication.findController.executeCommand('find', {
      query: finalString.toString(),
      phraseSearch: true,
      findPrevious: undefined,
      phraseSearch: true,
      highlightAll: true
    });
    // highlightAll = true;findHighlightAll
    var checked=document.getElementById('findHighlightAll');
    checked.checked = true;
    console.log('hi' + options.highlightAllCheckbox);
    this.opened = false;
    this.bar = options.bar || null;
    this.toggleButton = options.toggleButton || null;
    this.findField = options.findField || newValues.value;
    this.highlightAll = options.highlightAllCheckbox || null;
    this.caseSensitive = options.caseSensitiveCheckbox || null;
    this.findMsg = options.findMsg || newValues.value;
